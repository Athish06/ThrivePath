...
export const SessionRescheduleStepperModal: React.FC<SessionRescheduleStepperModalProps> = ({
  ...
  const renderModeCard = (
    ...
    return (
      <button
        type="button"
        className={`flex flex-col gap-3 rounded-xl border p-4 text-left transition-all hover:border-violet-400 hover:shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-1 ${
          isSelected
      ...
      >
        <div className="flex items-center gap-3">
          <span
            className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border text-lg font-semibold ${
              isSelected
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-5 dark:border-slate-700">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`rounded-lg bg-gradient-to-r from-violet-600 to-blue-600 px-5 py-2 text-sm font-semibold text-white shadow-lg transition-all hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-60 ${
              currentStep === 3 && selectedMode === 'cascade'
                ? 'min-w-[11rem]'
                : 'min-w-[9rem]'
            }`}
          >
      ...
      <Step>
        <div className="flex h-full flex-col gap-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-900/40">
            <label className="flex items-center justify-between gap-3 text-sm text-slate-700 dark:text-slate-200">
              <span className="flex items-center gap-2">
                ...
                Include weekend dates when shifting
              </span>
              <span className="inline-flex items-center gap-2">
                <span className="text-xs text-slate-500 dark:text-slate-400">{includeWeekends ? 'Yes' : 'No'}</span>
                <button
                  type="button"
                  onClick={() => {
                    if (!isCascadeSubmitting) {
                  ...
                  className={`relative h-6 w-11 rounded-full transition-colors ${
                    includeWeekends ? 'bg-violet-500' : 'bg-slate-300 dark:bg-slate-700'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${
                      includeWeekends ? 'translate-x-5' : 'translate-x-0.5'
                    }`}
                  />
                </button>
              </span>
            ...
            <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
              When weekends are excluded, sessions that fall on Saturday or Sunday will roll forward to the next Monday.
            </p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white/90 p-4 dark:border-slate-700 dark:bg-slate-800/80">
            ...
            {detail?.upcoming && detail.upcoming.count > 5 && (
              <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                + {detail.upcoming.count - 5} additional session(s)
              </p>
            )}
          </div>
        </div>
