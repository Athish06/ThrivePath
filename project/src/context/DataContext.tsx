...
export const DataProvider: React.FC<DataProviderProps> = ({ children }) => {
  ...
  const fetchTempStudents = useCallback(async () => {
    try {
      ...
      const token = localStorage.getItem('access_token');
      if (!token) throw new Error('No access token found');
      const response = await fetch('http://localhost:8000/api/temp-students', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const fetchTodaysSessions = useCallback(async () => {
    try {
      ...
      const token = localStorage.getItem('access_token');
      if (!token) throw new Error('No access token found');
      const response = await fetch('http://localhost:8000/api/sessions/today', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const getChildGoalsForLearner = useCallback(async (learnerId: string, options?: { force?: boolean }) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/students/${learnerId}/activities`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
